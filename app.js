...
let pendingLocalWrite = false; // true while our own change hasn't been confirmed by the cloud yet
  ...
  fbDocRef.onSnapshot((snap)=>{
    if(snap.exists){
      if(suppressNextSnapshot){
        ...
        if(cloudState){
          const incoming = JSON.parse(cloudState);
          // Never accept an incoming state that would "forget" the user
          // currently logged in on this device — that's always a sign of
          // a stale/incomplete snapshot racing with our own write, not a
          // legitimate removal (removals are explicit and go through
          // removeMemberCompletely, which we DO want to accept).
          const stillHasMe = !CURRENT_USER_ID || incoming.users.some(u=>u.id===CURRENT_USER_ID);
          if(stillHasMe){
            STATE = incoming;
            ensureWeekChecklist(STATE.currentWeekIndex);
            persistLocalCache();
            ...
            console.warn('[Sync] Ignored a cloud snapshot that did not include the current user — likely a stale write racing with registration/login.');
          }
        }
      }
    } else {
...
function pushStateToCloud(){
  if(!firebaseReady){ pendingLocalWrite = false; return; }
...
document.getElementById('registerForm').addEventListener('submit', async function(e){
  ...
  if(!user){
    ...
    // Log in FIRST, so pendingLocalWrite/CURRENT_USER_ID are set before we
    // touch the network at all — this closes the race where an incoming
    // snapshot could otherwise land between "push to cloud" and "save
    // session" and make it look like our own brand-new user "disappeared".
    saveSession(user.id);
    saveState();
...
function renderRoadmap(){
  ...
  document.querySelectorAll('[data-set-week-status]').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      ...
      console.log('[Roadmap] Mark-status button clicked:', btn.dataset.setWeekStatus, 'currentUser role:', currentUserNow && currentUserNow.role);
      if(!isLeader(currentUserNow)){
        if(!currentUserNow){
          // This should basically never happen now that we guard the
          // sync layer, but if it ever does, don't blame the user's
          // permissions for what is really a temporary sync hiccup.
